...
Time, mainly. The initial burst of energy that carried me through those early commits has been redirected to more immediate priorities - work, family, life. Building a programming language, even a small one, requires sustained focus and deep thinking. Right now, I don't have that bandwidth.

This isn't a failure or abandonment. It's recognition that side projects have seasons, and this one's season has passed for now.
...
- **REPLs are harder than they look** - the read-eval-print loop sounds straightforward until you implement it
- **Homoiconicity is powerful** - treating code as data opens up metaprogramming possibilities that feel like magic

I also learned that building a language solo is a marathon, not a sprint. The initial exploration phase is exhilarating - every commit adds visible functionality. But then you hit the "trough of sorrow" where the next features require solving hard, interconnected problems. That's where I am now, and that's where I need to pause.
